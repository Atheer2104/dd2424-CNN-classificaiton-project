...
if __name__ == "__main__":
	...
	ViT_resnet20 = model.ViT_resnet20(num_classes).to(device)
	ViT_resnet20.apply(he_initalization)

	ViT_resnet56 = model.ViT_resnet56(num_classes).to(device)
	ViT_resnet56.apply(he_initalization)

	ViT_resnet110 = model.ViT_resnet110(num_classes).to(device)
	ViT_resnet110.apply(he_initalization)
	...
	# print(summary(ViT_resnet20.to("cpu"), input_size=(3, 32, 32)))
	# print(summary(ViT_resnet56.to("cpu"), input_size=(3, 32, 32)))
	# print(summary(ViT_resnet110.to("cpu"), input_size=(3, 32, 32)))
	...
	print("------------- working on ViT ResNet20 -----------------")
	if use_her_parameters is False:
		print("optimizer is SGD")
		optimizer = torch.optim.SGD(
		ViT_resnet20.parameters(), lr=0.1, momentum=0.9, weight_decay=0.0001
		)
	else:
		print("optimizer is AdamW")
		optimizer = torch.optim.AdamW(
		ViT_resnet20.parameters(), lr=0.1, weight_decay=0.0001
		)
	...
	utils.evaluate(ViT_resnet20, test_dataloader, loss_fn, device)
	utils.plot_training_validation_loss_and_accuracy()
	utils.clear_histogram()

	print("------------- working on ViT ResNet56 -----------------")
	if use_her_parameters is False:
		print("optimizer is SGD")
		optimizer = torch.optim.SGD(
		ViT_resnet56.parameters(), lr=0.1, momentum=0.9, weight_decay=0.0001
		)
	else:
		print("optimizer is AdamW")
		optimizer = torch.optim.AdamW(
		ViT_resnet56.parameters(), lr=0.1, weight_decay=0.0001
		)
	...
	utils.evaluate(ViT_resnet56, test_dataloader, loss_fn, device)
	utils.plot_training_validation_loss_and_accuracy()
	utils.clear_histogram()

	print("------------------- working on ViT ResNet110 -----------------------")
	...
	if use_her_parameters is False:
		print("optimizer is SGD")
		optimizer = torch.optim.SGD(
			ViT_resnet110.parameters(), lr=0.01, momentum=0.9, weight_decay=0.0001
		)
	else:
		print("optimizer is AdamW")
		optimizer = torch.optim.AdamW(
			ViT_resnet110.parameters(), lr=0.01, weight_decay=0.0001
		)
	...
	utils.evaluate(ViT_resnet110, test_dataloader, loss_fn, device)
	utils.plot_training_validation_loss_and_accuracy()
	utils.clear_histogram()
